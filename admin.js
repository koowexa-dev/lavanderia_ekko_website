...
txt += '+'.repeat(80) + '\n';
txt += `| N ORDEN: ${ordenNo.padEnd(28)} Fecha Recepcion: ${fechaRecepcion.padEnd(20)} |\n`;
txt += `| Cliente: ${cliente.padEnd(30)} Telefono: ${telefono.padEnd(20)} |\n`;
txt += `| Direccion: ${direccion.padEnd(30)} Zona Mensajeria: ${zona.padEnd(17)} |\n`;
txt += `| Fecha Entrega: ${fechaEntrega.padEnd(50)} |\n`;
txt += '+'.repeat(80) + '\n';
txt += '| SERVICIOS POR CARGA (hasta 7kg) - Prelavado + Lavado + Secado + Doblado |\n';
txt += '+'.repeat(80) + '\n';
txt += '|  Peso    | Economico | Basico | Premium | Especial |\n';
txt += '|----------|-----------|--------|---------|----------|\n';
...
txt += '+'.repeat(80) + '\n';
txt += '| EXTRAS (+$300 c/u):\n';
txt += `|   ${extrasMap['Ropa COLOR']?'X':' '} Ropa COLOR     ${extrasMap['Perfume']?'X':' '} Perfume\n`;
txt += `|   ${extrasMap['Ropa BLANCA']?'X':' '} Ropa BLANCA   ${extrasMap['Perla Olor']?'X':' '} Perla Olor\n`;
txt += `|   ${extrasMap['Ropa OSCURA']?'X':' '} Ropa OSCURA   ${extrasMap['Fragancia']?'X':' '} Fragancia\n`;
...
txt += '+'.repeat(80) + '\n';
txt += '| PLANCHADO:\n';
txt += `|   Descripcion: ${planchadoTexto}\n`;
txt += `|   Costo: $${costoPlanchado}\n`;
txt += '+'.repeat(80) + '\n';
txt += '| MENSAJERIA:\n';
txt += `|   Recogida: ${msgRecogida?'Si':'No'}   Entrega: ${msgEntrega?'Si':'No'}\n`;
txt += `|   Direccion: ${direccionMensajeria}\n`;
...
txt += `|   Mensajeria: $${mensajeriaMonto}\n`;
txt += `|   TOTAL: $${total}\n`;
