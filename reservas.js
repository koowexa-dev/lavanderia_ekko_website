(function() {
    ...
    function actualizarResumen() {
        ...
        resumenDiv.innerHTML = `
            ...
            <p><strong>Ítems agregados (${currentPesosSeleccionados.length}/3):</strong> ${items}</p>
    ...
    function agregarPeso(rango, precio) {
        if (currentPesosSeleccionados.length >= 3) {
            window.showToast('Máximo 3 ítems en total', true);
            return false;
        }
        if (contadorRangos[rango] >= 3) {
            window.showToast(`No puedes agregar más de 3 veces el rango ${rango} kg`, true);
            return false;
        }
    ...
    function actualizarListaPesos() {
        ...
        if (currentPesosSeleccionados.length === 0) {
            container.innerHTML = '<p style="color:gray;">No hay ítems agregados. Selecciona rangos arriba.</p>';
            return;
        }
        ...
        for (let rango of rangosDisponibles) {
            ...
            if (btnContainer) {
                btnContainer.innerHTML = `
                    ...
                    <div class="rango-contador">${contador}/3</div>
                    <button type="button" class="btn-agregar-rango" data-rango="${rango}" ${currentPesosSeleccionados.length >= 3 || contador >= 3 ? 'disabled' : ''}>+</button>
                `;
                const btnAgregar = btnContainer.querySelector('.btn-agregar-rango');
                if (btnAgregar) {
                    btnAgregar.disabled = (currentPesosSeleccionados.length >= 3 || contador >= 3);
    ...
    function renderizarFormulario() {
        ...
        formContainer.innerHTML = `
            ...
            <div id="tab-servicio" class="tab-content" style="display:none;">
                ...
                <div class="form-group"><label>Selecciona rangos de peso (máx. 3 ítems, hasta 3 veces el mismo)</label>
        ...
        for (let rango of rangosDisponibles) {
            ...
            btnDiv.innerHTML = `
                ...
                <div class="rango-contador">0/3</div>
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            ...
            btn.addEventListener('click', () => {
                ...
                document.querySelectorAll('.tab-btn').forEach(b => b.style.fontWeight = '400');
        ...
        document.getElementById('enviarReservaBtn').addEventListener('click', async (e) => {
            ...
            if (!nombre || !telefono || !direccion || !carnet || !fechaHora) {
                window.showToast('Complete todos los datos del cliente', true);
                return;
            }
